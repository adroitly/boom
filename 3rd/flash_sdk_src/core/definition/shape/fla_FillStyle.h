...
namespace fla
{
    ...
    public:
        ...
        bool isInvisible() const;  // 是否全部是透明色
        ...
        void setFillData(RcObjPtr data)
        {
            _data = data;
        }
        ...
        uint16_t bitmapId() const
        {
            return data<BitmapData>().bitmapId;
        }

        const MatrixT& bitmapMatrix() const
        {
            return data<BitmapData>().matrix;
        }

        const GradientData& gradient() const
        {
            return data<GradientData>();
        }

        const SolidData& solidColor() const
        {
            return data<SolidData>();
        }
    ...
    private:
        ...
        {
            assert(_data);
            return boost::static_pointer_cast<RcWrapObj<T>>(_data)->val;
        }
        FillType _type;
        RcObjPtr _data;
    };
    ...
    {
        return (_type == FillType::RepeatingBitmap) || (_type == FillType::ClippedBitmap) ||
               (_type == FillType::NonSmoothedRepeatingBitmap) || (_type == FillType::NonSmoothedClippedBitmap);
    }

    template <int N>
    inline bool FillStyleBase<N>::isGradientStyle() const
    {
        return (_type == FillType::LinearGradient) || (_type == FillType::RadialGradient) ||
    ...
    bool FillStyleBase<N>::operator==(const FillStyleBase& rhs) const
    {
        ...
        if (isBitmapStyle())
        {
            return bitmapId() == rhs.bitmapId() && bitmapMatrix() == rhs.bitmapMatrix();
        }
    ...
    template <>
    inline bool FillStyleBase<1>::isInvisible() const
    {
        if (!isSolidStyle())
        {
            return false;
        ...
        auto data = this->solidColor();
        return data.val.isInvisible();
    }

    template <>
    inline bool FillStyleBase<2>::isInvisible() const
    {
        if (!isSolidStyle())
        {
            return false;
        ...
        auto data = this->solidColor();
        return data.start.isInvisible() && data.end.isInvisible();
    }
