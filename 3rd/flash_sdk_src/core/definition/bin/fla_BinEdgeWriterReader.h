...
namespace fla
{
    namespace bin
    {
        /* 形状的边有两种，lineto, 和curveTo, 其中
         ...
         | 7 | 6 | 5 | 4 | 3  2 | 1 0
         0，1  两位，保持边的类型，
         2, 3  表示压缩之后每个float使用的字节数目,
         0表示无压缩，1使用1字节，2表示使用2字节，3表示使用4字节
        ...
        public:
            ...
            EdgeWriterReader(float scale, const Point& curPt);
        ...
        private:
            ...
            Offset updateCurrent(Point last);
            void readCurPoint(const uint8_t*& ptr,
                              bool hasX,
                              bool hasY,
                              const FlagsInfo& flags);
            void writeOffset(Data& data,
                             const Offset& offset,
                             const Point& pt,
                             const FlagsInfo& flags);
