...
void Parser::ParseEnum(bool is_union) {
  ...
  do {
    std::string attriName = attribute_;
    std::vector<std::string> docComment = doc_comment_;
    Expect(kTokenIdentifier);
    auto prevsize = enum_def.vals.vec.size();
    auto value = enum_def.vals.vec.size()
      ? enum_def.vals.vec.back()->value + 1
      : 0;
    auto &ev = *new EnumVal(attriName, value);
    if (enum_def.vals.Add(attriName, &ev))
      Error("enum value already exists: " + attriName);
    ev.doc_comment = docComment;
    if (is_union) {
      ev.struct_def = LookupCreateStruct(attriName);
    }
    if (IsNext('=')) {
