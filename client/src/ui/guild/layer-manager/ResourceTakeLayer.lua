...
local M = class("ui.guild.layer-manager.ResourceTakeLayer", function()
...
function M:_createBackground()
    local background = BackgroundLayer.new(self, "取出资源")
...
function M:_createButton()
    local confirmButton = CommonUI:createTextButton(self, "取出资源", "ui/login/login-bt.png")
...
function M:onTableViewCellAtIndex(tableView, idx)
    ...
    if not cell then
        cell = ResourceOperateCell.new(Enum.ResourceOperateType.Take)
    end
