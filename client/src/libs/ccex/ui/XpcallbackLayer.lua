local M = class("ccex.ui.XpcallbackLayer", function(msg, traceback)
    color = color or cc.c4b(0, 0, 0, 150)
...
function M:ctor(msg, traceback)
    ...
    Layout:layout(messageLabel, Screen.size, Layout.center)

    if Screen.size.width < Screen.size.height then 
    ...
    self:addChild(menu)
    menu:setPosition(100, Screen.height - 40)
end
